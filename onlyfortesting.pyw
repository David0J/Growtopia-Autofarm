...
layout = [
    [Psg.Text("Welcome to David's auto-farming menu!")],
    [Psg.Text("Farming:")],
    [Psg.Button(image_data=lgrid, key='Laser Grids'),
     Psg.Button(image_data=chand, key='Chandeliers'),
     Psg.Button(image_data=p1s1g, key='Grass/Pepper/Sugar')],
    [Psg.Text("Harvesting:")],
    [Psg.Button(image_data=sorow, key='Harvesting With Sorrow')],
    [Psg.Button("Tutorial"), Psg.Button('', image_data=brows, key='web')],
    [Psg.Button("Start/Stop"), Psg.Button("Close")]
]

window = Psg.Window("Auto-Farmer", layout, finalize=True, grab_anywhere=True)
...
while True:
    ...
    if event in ('Laser Grids', 'Chandeliers', 'Grass/Pepper/Sugar', 'Harvesting With Sorrow'):
        if event == 'Chandeliers':
            punch_wait, punch_time = 300, 70
        elif event == 'Grass/Pepper/Sugar':
            punch_wait, punch_time = 300, 100
        elif event == 'Laser Grids':
            punch_wait, punch_time = 300, 85
        elif event == 'Harvesting With Sorrow':
            punch_wait, punch_time = 0, 500
        running = True
        
        
    if event == 'web':
        webbrowser.open_new(r'https://davidjawhar.com')
    
    if event == 'Tutorial':
        webbrowser.open_new(r'https://david0j.github.io/Growtopia-Autofarmer/')
